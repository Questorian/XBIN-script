...
# 2009-01-19 - changed the datetime check to localtime(), instead of gmtime(), which
#   has returning the wrong localtimes for the recorded sound files
...
my $workflow_dir = 'D:\\QS\\_td\\inbox\\AUDIO\\inbox\\non-music\\VOX';

# should we get a list of existing photo names from the library to ensure that we do not get the same
# name twice (a name collision)
my $NoCollision;
...
if ($NoCollision) {
    print
"Checking existing photos in library to avoid name collisions: $album_root\n";
    BuildPNL();
    my $num = @PNL;
...
print "Starting the explorer...\n";

# call the explorer to review the imported files
system("explorer $workflow_dir");
exit;
...
sub MovePhoto() {
    my (
        $file,            $file_created, $ctime, $seq,
        $target_filename, $basename,     $path,  $new_filename,
        $bpath,           $suffix
    ) = @_;
    ...
    if ( $file =~ /THM/ ) { ; }
        ...
        ( $basename, $bpath, $suffix ) = fileparse( $file, ( '.wma', '.WMA' ) );
    ...
    #              sprintf( "%02d%02d%02d%03d.jpg", $syear, $mon, $mday, $seq );
            $target_filename = sprintf( "vox_%d-%02d-%02dT%02d_%02d_%02d%s",
                $lyear, $mon, $mday, $hour, $min, $sec, $suffix );
            $seq++;
